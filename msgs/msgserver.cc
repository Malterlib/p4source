...
ErrorId MsgServer::UseInteg            = { ErrorOf( ES_SERVER, 246, E_FAILED, EV_USAGE, 0 ), "Usage: %'integrate [ -c changelist# -D<flags> -f -h -K -m max -n -Obr -q -r -s from -v ] [ -b branch to... | from to ] [ [-As | -Af] -S stream [ -P parent ] files... ]'%" } ;
ErrorId MsgServer::UseIntegb           = { ErrorOf( ES_SERVER, 247, E_FAILED, EV_USAGE, 0 ), "Usage: %'integrate [ -c changelist# -D<flags> -f -h -K -m max -n -Obr -q -r -s from -v ] -b branch [ to... ]'%" } ;
ErrorId MsgServer::UseIntegS           = { ErrorOf( ES_SERVER, 507, E_FAILED, EV_USAGE, 0 ), "Usage: %'integrate [ -c changelist# -D<flags> -f -h -K -m max -n -Obr -q -r -s from -v ] [-As | -Af]  -S stream [ -P parent ] [ files... ]'%" } ;
...
ErrorId MsgServer::UseStorage          = { ErrorOf( ES_SERVER, 1009, E_FAILED, EV_USAGE, 0 ), "Usage: %'storage [ -w ] | [ -v [ -q ] | -U [ -q ] | -u [ -f ] | -d [ -q ] [ -y ] [ -D secs ] [ -t target ] ] [ -F filter -T fields -m max ] [ -c change ] archives... | -l start|pause|restart|wait|status|cancel //depotdirectory/...'%" } ;
...
ErrorId MsgServer::UseSync             = { ErrorOf( ES_SERVER, 277, E_FAILED, EV_USAGE, 0 ), "Usage: %'sync [ -f -k -K -n -N -p -q -r -s ] [-m max] [files...]'%" } ;
