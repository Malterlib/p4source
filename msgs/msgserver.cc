...
ErrorId MsgServer::UseChanges          = { ErrorOf( ES_SERVER, 213, E_FAILED, EV_USAGE, 0 ), "Usage: %'changes [-i -t -l -L -f -r -c client -e changelist# -m count -s status -u user] [files...] [--stream | --nostream]'%" } ;
...
ErrorId MsgServer::UseDbstat           = { ErrorOf( ES_SERVER, 344, E_FAILED, EV_USAGE, 0 ), "Usage: %'dbstat [ -h -f | -p [ -w worklevel ] [ -n nofiles ] ] { -a | dbtable ... }\n       dbstat -s'%" } ;
